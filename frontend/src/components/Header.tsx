...
const Header: React.FC<HeaderProps> = ({setAuthModal}) => {


    const activateModal = (e: React.MouseEvent<HTMLButtonElement>) => {
        e.preventDefault();
        setAuthModal(true);
    }
        ...
        <>
            ...
            <header className="header flexCenter">
                <ul className="header__list">
                    ...
                    <li className="header__item flexCenter">
                        <MyButton onClick={(e: React.MouseEvent<HTMLButtonElement>) => activateModal(e)} style={{height: '', fontSize: '18px' }}>Авторизация</MyButton>
                    </li>
                </ul>
            </header>
        </>
